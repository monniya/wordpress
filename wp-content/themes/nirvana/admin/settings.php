...
function cryout_setting_frontslider2_fn() {
	...
	echo "<div><small>".__("Only the slides with a defined image will become active and visible in the live slider.<br>When using slides from posts, make sure the selected posts have featured images.<br>Read the FAQs for more info.","nirvana")."</small></div>";
     ...
     <div class="underSelector">
          ...
          <span id="slider-post-number"><?php _e('Number of posts to show:','nirvana'); ?>
               <input id='nirvana_slideNumber' name='nirvana_settings[nirvana_slideNumber]' size='3' type='text' value='<?php echo esc_attr( $nirvanas['nirvana_slideNumber'] ) ?>' />
          </span>
...
<?php	echo "<div class='slmini'><b>".__("Columns per row:","nirvana")."</b> ";
	$items = array ("1", "2" , "3" , "4");
	...
	global $nirvanas;
     ...
     echo "<div style='width:100%;'><span>".__("Text for the Presentation Page","nirvana")."</span><small>".
          __("More text for the Presentation Page. The top title is just below the slider, the second title is below the columns. A text area supporting HTML tags and shortcodes below each title<br>".
     	   "It's all optional so leave any input field empty to not dispaly it.","nirvana")."</small></div>";
...
function cryout_setting_searchbar_fn() {
	...
	$items = array( "top");
	$itemsare = array( __("Top Menu","nirvana"));
	$i=0;
	foreach($items as $item):
...
function  cryout_setting_contentmargins_fn() {
	...
	echo "<div><small>".__("The margin between the content and the menu. It can be set to 0px if you want the content area and menu to join.","nirvana")."</small></div><br><br>";
	...
	echo "<div><small>".__("The left/right padding around the content. Should be set to 10px or less for designs without a content color.","nirvana")."</small></div>";
}
...
function cryout_setting_social_master($i) {
	...
	$cryout_social_small = array (
		...
		'',__("You can also choose if you want the link to open in a new window and what title to dispaly while hovering over the icon.",'nirvana'),
