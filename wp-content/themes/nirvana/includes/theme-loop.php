...
function nirvana_excerpt_length_chars( $output ) {
	...
	if ($output && !is_attachment())	$output = substr($output, 0, $nirvana_excerptlength).$nirvana_excerptdots.nirvana_continue_reading_link();
...
function nirvana_posted_on() {
	 ...
	 if (is_single()) {$nirvana_blog_show = $nirvana_single_show;}

	// Post Author
	$output="";
	 if ($nirvana_blog_show['author']) {
     $output .= sprintf( '<span class="author vcard" ><i class="icon-author icon-metas" title="'.__( 'Author ','nirvana'). '"></i>  <a class="url fn n" href="%1$s" title="%2$s">%3$s</a> <span class="bl_sep">&#8226;</span></span>',
	 ...
	 if ($nirvana_blog_show['date'] || $nirvana_blog_show['time'] ) { 
		$separator="";$date="";$time ="";
		if ( $nirvana_blog_show['date'] && $nirvana_blog_show['time'] ) {$separator = " - ";}
		if ($nirvana_blog_show['date']) { $date = get_the_date(); }
		if ($nirvana_blog_show['time']) { $time = esc_attr( get_the_time() ); }
		$output.='<span class="onDate date updated"><i class="icon-time icon-metas" title="'.__("Date", "nirvana").'"></i><a href="'.get_permalink().'" rel="bookmark">'.$date.$separator.$time.'</a></span>';
	}
	// Post categories
     if ($nirvana_blog_show['category']) { 
			$output .= '<span class="bl_categ"><i class="icon-folder-open icon-metas" title="'.__("Categories", "nirvana").'"></i>'.get_the_category_list( ', ' ).'</span> ' ;
		}
		echo $output;
...
function nirvana_posted_after() { 
	...
	$tag_list = get_the_tag_list( '', ', ' );
     if ( $tag_list && ($nirvana_blog_show['tag']) ) { ?>
		<span class="footer-tags"><i class="icon-tag icon-metas" title="<?php _e( 'Tags','nirvana'); echo '"> </i>'.$tag_list; ?> </span>
     <?php }
...
function nirvana_content_nav( $nav_id ) {
	...
	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav id="<?php echo $nav_id; ?>" class="navigation">
			<div class="nav-previous"><?php next_posts_link( __( '<i class="meta-nav-prev"></i> <span>Older posts</span>', 'nirvana' ) ); ?></div>
			<div class="nav-next"><?php previous_posts_link( __( '<span>Newer posts</span> <i class="meta-nav-next"></i>', 'nirvana' ) ); ?></div>
		</nav><!-- #nav-above -->
	<?php endif;
}; // nirvana_content_nav()
...
if ($nirvana_fcrop) add_image_size( 'custom', $nirvana_fwidth, $nirvana_fheight, true );
                else add_image_size( 'custom', $nirvana_fwidth, $nirvana_fheight );
...
function nirvana_set_featured_thumb() {
	...
	foreach ($nirvanas as $key => $value) { ${"$key"} = $value; }
     ...
     if ( function_exists("has_post_thumbnail") && has_post_thumbnail() && $nirvana_fpost=='Enable')
			...
			echo '<a title="'.the_title_attribute('echo=0').'" href="'.get_permalink().'" ><img width="'.$nirvana_fwidth.'" title="" alt="" class="align'.strtolower($nirvana_falign).' post_thumbnail" src="'.$image_src.'"></a>' ;

};
endif; // nirvana_set_featured_thumb

if ($nirvana_fpost=='Enable' && $nirvana_fpostlink) add_filter( 'post_thumbnail_html', 'nirvana_thumbnail_link', 10, 3 );
