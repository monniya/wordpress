...
<?php  } // form() function
  ...
  function widget($args, $instance) { 
	...
	global $nirvana_column_counter; // globabl counter for incrementing further
	$blank="";
	...
	if($instance['image']) : 
		...
		<div class="column<?php echo ($counter%$nirvana_nrcolumns)?$counter%$nirvana_nrcolumns:$nirvana_nrcolumns; ?>"> 
					
			<?php if ($instance['image']) :	?>
				<div class="column-image">
					<img  src="<?php echo esc_url($instance['image']) ?>" id="columnImage<?php echo $counter; ?>" alt="" />
						<div class="column-image-inside">	
						<a  class="column-link" <?php echo $blank;?> href="<?php echo esc_url($instance['link']) ?>">	</a>
						<div class="column-image-inside-centered">
							  <?php if ($instance['title']) { 
										echo "<a ".$blank." href='". esc_url($instance['link']) ."'><h3 class='column-header-image'>".$instance['title']."</h3></a>"; 
								} ?>
						 <div class="column-text-separator"></div>
							<?php if ($instance['text']) : ?>		
									<div class="column-text">	<?php echo do_shortcode($instance['text']); ?>	</div>
							<?php endif; ?>
